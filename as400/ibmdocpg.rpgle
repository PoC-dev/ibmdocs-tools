     ...
     H* see there for further details.
     H*
     ...
     H* ITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License
     H* for more details.
     H*
     ...
     H*     38: Do not display DATE_ADDED (if it's NULL).
     H*         => This is for the "display document types" SFL only!
     ...
     H*     45: Hide date_added, if it's NULL. (DSPF)
     H* 50..53: SFL Sorting.
     H* 60..69: Detail record cursor placement. (DSPF)
     H*- Other Stuff:
     H*     71: READC from Subfile EOF.
     H*     72: SETLL of POSTO found no records.
     H*     73: CHAIN for a pre-existing document number failed.
     H*         => For duplicating into an existing document number!
     H*     75  SETLL for document type no records,
     H*         READE for document type record EOF.
     H*         => For populating FT field in SFL.
     H*     76: SETLL for document type exact match.
     H*         => For populating FT field in SFL.
     ...
     FIBMDOCPF  UF A E           K DISK    INFDS(FINFDS)
     FIBMDOCTYPFIF   E           K DISK
     F*
     ...
     FIBMDOCPOS1IF   E           K DISK
     FIBMDOCPOS2IF   E           K DISK
     FIBMDOCPOS3IF   E           K DISK
     F*
     F* This LF is solely to check if we can safely overwrite a record.
     FDOCNBRLF  IF   E           K DISK    PREFIX(X_)
     F*
     F* Display file with multiple subfiles among other record formats.
     FIBMDOCDF  CF   E             WORKSTN
     F                                     SFILE(MAINSFL:SFLRCDNBR)
     F                                     SFILE(DLTSFL:SFLDLTNBR)
     F                                     SFILE(DETAILSFL:DETAILNBR)
     F*
     F*************************************************************************
     ...
     D*  different kind).
     DT_DOCTBL       E DS                  EXTNAME(IBMDOCPF:DOCTBL) INZ
     D* To actually store stuff, we need separate space. V4 doesn't know about
     D*  LIKEDS, so we need to build the struct by hand.
     D* Note: We (ab)use the fields for storing the POSTO search string. Should
     D*  not interfere with our established logic, which is only used when OPT
     D*  selection are handled.
     DC_DOCTBL         DS                  INZ
     D C_TITLE                             LIKE(TITLE)
     D C_DOCNBR                            LIKE(DOCNBR)
     D C_RELEASED                          LIKE(RELEASED)
     D*
     ...
     C*
     C                   SELECT
     C     SORTKEY       WHENEQ    *ZERO
     C                   MOVEL     *BLANK        C_DOCNBR
     C                   MOVEL     POSTO         C_DOCNBR
     C     C_DOCNBR      SETLL     FWDPOS1                            72
     C     SORTKEY       WHENEQ    1
     C                   MOVEL     *BLANK        C_TITLE
     ...
     C     *IN11         WHENEQ    *ON
     C*
     C* Reset INxx for DSPF underline.
     C                   MOVEA     '000'         *IN(50)
     C*
     C* Prepare SORTKEY for later choosing of input file, and calculate
     C*  which INxx to set on for DSPF underline.
     C     SORTKEY       IFLT      2
     C                   ADD       1             SORTKEY
     C                   ELSE
     C                   Z-ADD     *ZERO         SORTKEY
     C                   ENDIF
     C*
     C     SORTKEY       ADD       50            DSPFUL
     C                   MOVE      *ON           *IN(DSPFUL)
     C*
     C* Write and reload, so our change is reflected.
     C                   EXSR      LOADDSPSFL
     C*
     C*------------------
     ...
     C*
     C* Use a different LF for different sorting.
     C                   SELECT
     C     SORTKEY       WHENEQ    *ZERO
     C     *LOVAL        SETLL     FWDPOS1
     C     SORTKEY       WHENEQ    1
     C     *LOVAL        SETLL     FWDPOS2
     C     SORTKEY       WHENEQ    2
     C     *LOVAL        SETLL     FWDPOS3
     C                   ENDSL
     C                   MOVE      *OFF          *IN72
     C*
     C                   ELSE
     ...
     C                   SELECT
     C     SORTKEY       WHENEQ    *ZERO
     C     DOCNBR        SETLL     FWDPOS1                            72
     C     SORTKEY       WHENEQ    1
     C     SORTKEY1      SETLL     FWDPOS2                            72
     C     SORTKEY       WHENEQ    2
     C     SORTKEY2      SETLL     FWDPOS3                            72
     C                   ENDSL
     C                   ENDIF
     C*
     ...
     C                   MOVEL     *BLANK        TITLE$
     C                   EVAL      STRLEN_CUR=%LEN(%TRIMR(TITLE))
     C     STRLEN_CUR    IFGT      STRLEN_MAX
     C     STRLEN_MIN    SUBST     TITLE:1       TITLE$
     C                   CAT       '...':0       TITLE$
     C                   ELSE
     C     STRLEN_MAX    SUBST     TITLE:1       TITLE$
     C                   ENDIF
     C*
     C* Check for NULL fields and overwrite with *BLANK so fields appear empty.
     C                   EXSR      CHECKNULL
     C*
     C* Format field showing which formats of the document we have.
     C                   MOVE      *BLANK        DOCTYPE$
     C*
     C* Check if (exactly matching) records exist.
     C     DOCNBR        SETLL     DOCTYPTBL                          75  76
     C     *IN75         IFEQ      *OFF
     C     *IN76         ANDEQ     *ON
     C*
     C* Read all records and copy "what do we have" flag accordingly.
     C     *ZERO         DOWEQ     *ZERO
     C     DOCNBR        READE(N)  DOCTYPTBL                              75
     C     *IN75         IFEQ      *ON
     C                   LEAVE
     C                   ENDIF
     C*
     C     DOCTYPE       IFNE      *BLANK
     C                   CAT       DOCTYPE:0     DOCTYPE$
     C                   ENDIF
     C*
     C                   ENDDO
     C*
     ...
     C                   SELECT
     C     SORTKEY       WHENEQ    *ZERO
     C     DOCNBR        SETLL     FWDPOS1
     C     SORTKEY       WHENEQ    1
     C     SORTKEY1      SETLL     FWDPOS2
     C     SORTKEY       WHENEQ    2
     C     SORTKEY2      SETLL     FWDPOS3
     C                   ENDSL
     C                   EXSR      LOADDSPSFL
     C                   MOVE      *OFF          *IN35
     C                   ELSE
     ...
     C                   SELECT
     C     SORTKEY       WHENEQ    *ZERO
     C     *HIVAL        SETGT     BCKPOS1
     C     SORTKEY       WHENEQ    1
     C     *HIVAL        SETGT     BCKPOS2
     C     SORTKEY       WHENEQ    2
     C     *HIVAL        SETGT     BCKPOS3
     C                   ENDSL
     C                   MOVEA     '10'          *IN(34)
     C                   ENDIF
     C*
     ...
     C*
     C* Prepare SORTKEY defaults.
     C                   MOVE      *ZERO         SORTKEY
     C     SORTKEY       ADD       50            DSPFUL
     C                   MOVE      *ON           *IN(DSPFUL)
     C*
     C* For proper positioning when handling records.
     C     SORTKEY1      KLIST
     C                   KFLD                    TITLE
     C                   KFLD                    DOCNBR
     C     SORTKEY2      KLIST
     ...
     C*  message.
     C* FIXME: Additonal conditions might be introduced, to prevent mishaps.
     C*
     C* Show message when we can't find the record anymore.
     C     *IN81         IFEQ      *ON
     C                   WRITE     DETAILBTM
     ...
     C*
     C* Delete record.
     C     DOCNBR        DELETE(E) DOCTBL
     C                   EVAL      FSTAT=%STATUS(IBMDOCPF)
     C     FSTAT         IFGT      *ZERO
     C                   EXSR      SETERRIND
     C                   MOVE      *ON           *IN99
     C                   UPDATE    DLTSFL
     ...
     C     *IN78         IFEQ      *ON
     C     DOCNBR        CHAIN(EN) DOCTBL
     C                   ELSE
     C     DOCNBR        CHAIN(E)  DOCTBL
     C                   ENDIF
     C*
     C                   EVAL      FSTAT=%STATUS(IBMDOCPF)
     C     FSTAT         IFGT      *ZERO
     ...
     C                   EXSR      CHECKNULL
     C*
     C* Undo hide output only NULL field when duplicating.
     C     *IN78         IFEQ      *ON
     C                   MOVE      *OFF          *IN45
     C                   ENDIF
     C*
     ...
     C                   EXSR      SETNULL
     C*
     C* We regularly have records with only a newer revision of a document but
     C*  without a title (yet). Normally we must manually delete the "empty"
     C*  record and duplicate the one with a different revision. This is
     C*  cumbersome.
     C* Solution: If there is a record already existing with a given DOCNBR, and
     C*  *BLANKs in the title, delete that one, so we can safely write.
     C* FIXME: Show message that we did overwrite instead of creating a new rec.
     C* FIXME: Implement error handling.
     C     *IN78         IFEQ      *ON
     C     DOCNBR        CHAIN     NBRCHKTBL                          73
     C     *IN73         IFEQ      *OFF
     C     X_TITLE       ANDEQ     *BLANK
     C     X_DOCNBR      DELETE    DOCTBL
     C                   ENDIF
     C                   ENDIF
     C*
     ...
     C     LOADTYPSFL    BEGSR
     C*
     C* Save current SFL *INs so we can freely set ours.
     C                   MOVEA     *IN(31)       SAVIND(1)
     C                   MOVEA     '000000000'   *IN(31)
     C*
     C* Show document types if available.
     C     DOCNBR        SETLL     DOCTYPTBL                          75  76
     C     *IN75         IFEQ      *OFF
     C     *IN76         ANDEQ     *ON
     C*
     C* Reset SFL state to before the first load.
     C                   MOVEA     '001000000'   *IN(31)
     C                   MOVE      *ZERO         DETAILNBR
     C                   WRITE     DETAILCTL
     C                   MOVE      *OFF          *IN33
     C*----------------------------
     C* Read loop start.
     C     *ZERO         DOWEQ     *ZERO
     C     DOCNBR        READE(N)  DOCTYPTBL                              34
     C     *IN34         IFEQ      *ON
     C                   LEAVE
     C                   ENDIF
     C*
     C* Condition Data for SFL.
     C                   SELECT
     C     DOCTYPE       WHENEQ    'P'
     C                   MOVEL     'PDF '        DOCTYPE#
     C     DOCTYPE       WHENEQ    'B'
     C                   MOVEL     'BOOK'        DOCTYPE#
     C                   ENDSL
     C*
     C                   IF        %NULLIND(DATE_ADDED)
     C                   MOVE      *ON           *IN38
     C                   ELSE
     C                   MOVE      *OFF          *IN38
     C                   ENDIF
     ...
     C                   Z-ADD     1             DETAILNBR
     C*
     C* Finally allow to show all the data on the display.
     C                   MOVE      *ON           *IN32
     C                   WRITE     DETAILBTM
     C                   WRITE     DETAILCTL
     C*
     C* If SETLL found no records.
     C                   ELSE
     C                   WRITE     DETAILND
     C                   ENDIF
     C*
     ...
     C                   MOVEA     '0000000000'  *IN(60)
     C     CSREC         IFEQ      'DETAILCTL'
     C                   SELECT
     C     CSFLD         WHENEQ    'DOCNBR'
     C                   MOVE      *ON           *IN60
     C     CSFLD         WHENEQ    'TITLE'
     C                   MOVE      *ON           *IN61
     C     CSFLD         WHENEQ    'RELEASED'
     C                   MOVE      *ON           *IN62
     C     CSFLD         WHENEQ    'SUBTITLE'
     C                   MOVE      *ON           *IN63
     C                   ENDSL
     C                   ENDIF
     C*
     ...
     C*  compiler flags!
     C*
     C                   IF        %NULLIND(SUBTITLE)
     C                   MOVEL     *BLANK        SUBTITLE
     C                   ENDIF
     C*
     C                   ENDSR
     C*************************************************************************
     ...
     C*  compiler flags!
     C*
     C     SUBTITLE      IFEQ      *BLANK
     C                   EVAL      %NULLIND(SUBTITLE) = *ON
     C                   ENDIF
     C*
     C                   ENDSR
     C*************************************************************************
